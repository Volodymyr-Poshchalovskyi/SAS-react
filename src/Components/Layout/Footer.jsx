...
const FooterLink = ({ path, text }) => (
  <li>
    <Link
      to={path}
      className="font-bold text-white text-sm uppercase tracking-widest hover:underline"
    >
      {text}
    </Link>
  </li>
...
const ColumnSection = ({ title, links, path }) => {
  ...
  const originalsClasses =
    'font-bold text-white text-sm uppercase tracking-widest hover:underline';
  const titleClasses = originalsClasses + ' mb-6';

  const subLinkClasses =
    'font-light text-sm text-gray-400 hover:text-white transition-colors';
      ...
      {path ? (
        <Link
          ...
          className={
            isOriginals
              ? titleClasses
              : 'font-bold text-white text-sm uppercase tracking-widest mb-6 hover:underline'
          }
        >
          {title}
        </Link>
      ) : (
        <span className="text-xs font-bold uppercase tracking-widest mb-6 block">
          {title}
        </span>
      )}
      <ul
        className={`space-y-3 list-none p-0 m-0 ${isOriginals ? 'mt-4' : ''}`}
      >
