...
// * Data structure for the privacy policy content.
// * This structure allows for different content types like paragraphs, lists, and addresses.
const policyData = [
  // Intro Section
  {
    type: 'intro', // * Custom type for potential specific styling (though not used currently)
    content: [
...
// ========================================================================== //
// ! MAIN COMPONENT: PrivacyPolicyPage
// ========================================================================== //

/**
 * ? PrivacyPolicyPage Component
 * Renders the static privacy policy content based on the `policyData` structure.
 */
export default function PrivacyPolicyPage() {
  return (
    ...
    <div className="bg-white text-black">
      ...
      <div className="w-[85%] mx-auto px-6 pt-[150px] pb-16 sm:pb-24 lg:px-8"> {/* // * pt-[150px] likely offsets a fixed header */}
        {/* // * Page Title */}
        <h1 className="font-['Inter'] text-3xl sm:text-4xl font-bold uppercase tracking-wider text-center mb-12">
          Privacy Policy
        </h1>

        {/* // * Container for policy text with specific font and line height */}
        <div className="font-['Inter'] text-xs leading-tight"> {/* // * Tighter line height */}
          {/* // * Map through each section in policyData */}
          {policyData.map((section, sectionIndex) => (
            <React.Fragment key={sectionIndex}>
              {/* // * Render the section title if it exists */}
              {section.title && (
                 // * Use a paragraph for title for consistent spacing, could also be h2/h3
                 <p className="mb-4 font-semibold text-sm mt-6 first:mt-0"> {/* Add font-semibold, text-sm, mt-6 */}
                   {section.title}
                 </p>
              )}

              {/* // * Map through the content items within the section */}
              {section.content.map((item, itemIndex) => {
                const uniqueKey = `${sectionIndex}-${itemIndex}`; // * Create a unique key for React

                // * Handle simple string content items
                ...
                // * Handle structured content items based on 'type'
                switch (item.type) {
                  case 'paragraph':
                    return (
                      <p key={uniqueKey} className="mb-4">
                  ...
                  case 'address':
                    // * Render address block using dangerouslySetInnerHTML for <br /> tags
                    return (
                      <p
                        key={uniqueKey}
                        className="mb-4" // * Apply margin bottom
                        // * Be cautious with dangerouslySetInnerHTML; ensure content is trusted
                        dangerouslySetInnerHTML={{ __html: item.content.join('') }}
                      />
                    );
                  case 'intro': // * Handle intro type (currently renders same as string paragraphs)
                    // * Check if item is an array (as in the example) or just a string
                    if (Array.isArray(item.content)) {
                        return item.content.map((introPara, introIndex) => (
                             <p key={`${uniqueKey}-${introIndex}`} className="mb-4">{introPara}</p>
                        ));
                    } else if (typeof item.content === 'string') {
                         return <p key={uniqueKey} className="mb-4">{item.content}</p>;
                    }
                    return null; // * Fallback if intro content format is unexpected
                  default:
                    // * Render simple strings within the intro section as paragraphs
                    if (section.type === 'intro' && typeof item === 'string') {
                         return <p key={uniqueKey} className="mb-4">{item}</p>;
                    }
                    // * Return null for unknown types or handle default case if needed
                    console.warn("Unknown policy item type:", item.type);
                    return null;
                }
              })}
            </React.Fragment>
