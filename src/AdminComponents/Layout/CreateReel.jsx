...
const ReelPartialForm = ({ reel, onUpdate, onRemove }) => { const { id, title, selectedFile, customPreview } = reel; const [isDragging, setIsDragging] = useState(false); const fileInputRef = useRef(null); const previewFileInputRef = useRef(null); const mainPreviewUrl = useMemo(() => { return selectedFile ? URL.createObjectURL(selectedFile) : null; }, [selectedFile]); const customPreviewUrl = useMemo(() => { return customPreview ? URL.createObjectURL(customPreview) : null; }, [customPreview]); useEffect(() => { return () => { if (mainPreviewUrl) URL.revokeObjectURL(mainPreviewUrl); }; }, [mainPreviewUrl]); useEffect(() => { return () => { if (customPreviewUrl) URL.revokeObjectURL(customPreviewUrl); }; }, [customPreviewUrl]); const handleChange = (field, value) => { onUpdate(id, { ...reel, [field]: value }); }; const handleFileValidation = (file, types = ['image/', 'video/']) => { if (!file) return false; const isValid = types.some(type => file.type.startsWith(type)); if (!isValid) { alert(`Please upload a valid file type (${types.join(', ')}).`); return false; } return true; }; const handleFileSelect = (file) => { if (file && handleFileValidation(file)) { handleChange('selectedFile', file); } }; const handleDrop = (e) => { e.preventDefault(); setIsDragging(false); handleFileSelect(e.dataTransfer.files?.[0]); }; const handleFileInputChange = (e) => { handleFileSelect(e.target.files?.[0]); }; const handleRemoveFile = () => handleChange('selectedFile', null); const handleCustomPreviewSelect = (e) => { const file = e.target.files[0]; if (handleFileValidation(file, ['image/'])) { handleChange('customPreview', file); } }; const removeCustomPreview = () => handleChange('customPreview', null); return ( <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-4 space-y-8 relative mb-8"> <button type="button" onClick={() => onRemove(id)} className="absolute -top-4 -right-4 z-10 p-2 bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-75" aria-label="Remove Media" > <Trash2 size={18} /> </button> <FormSection> <FormField label="Title" required> <input type="text" value={title} onChange={(e) => handleChange('title', e.target.value)} placeholder="Enter unique title..." className={inputClasses} required /> </FormField> </FormSection> <FormSection title="Upload Content"> <div onDragOver={(e) => {e.preventDefault(); setIsDragging(true);}} onDragLeave={(e) => {e.preventDefault(); setIsDragging(false);}} onDrop={handleDrop} className={`relative flex flex-col items-center justify-center w-full aspect-[16/9] border-2 border-slate-300 border-dashed rounded-lg cursor-pointer transition-colors dark:border-slate-600 ${isDragging ? 'border-teal-500 bg-teal-50 dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'}`}> {!selectedFile ? ( <div className="flex flex-col items-center justify-center text-center"> <UploadIcon /> <p className="mb-2 text-sm text-slate-500 dark:text-slate-400"><span className="font-semibold">Drag & Drop file</span></p> <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">or</p> <button type="button" onClick={() => fileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-teal-500 text-white rounded-md hover:bg-teal-600">Choose File</button> </div> ) : ( <div className="w-full h-full p-2"> {mainPreviewUrl && selectedFile.type.startsWith('image/') && <img src={mainPreviewUrl} alt="Preview" className="w-full h-full object-contain rounded-md" />} {mainPreviewUrl && selectedFile.type.startsWith('video/') && <video src={mainPreviewUrl} controls className="w-full h-full object-contain rounded-md" />} <button type="button" onClick={handleRemoveFile} className="absolute top-2 right-2 p-1.5 bg-black/50 text-white rounded-full hover:bg-black/70 transition-colors" aria-label="Remove file"> <X size={16}/> </button> </div> )} </div> <input id={`dropzone-file-${id}`} type="file" className="hidden" ref={fileInputRef} onChange={handleFileInputChange} accept="image/*,video/*" /> </FormSection> <FormSection title="Preview" hasSeparator={false}> {!customPreviewUrl ? ( <div className="text-center text-slate-400 py-8"> <p className="mb-4">Here will be shown your preview</p> <button type="button" onClick={() => previewFileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600"> Choose preview </button> </div> ) : ( <div className="flex flex-col items-center"> <div className="w-full flex justify-center mb-4"> <img src={customPreviewUrl} alt="Final Preview" className="w-full h-auto max-w-2xl max-h-[500px] rounded-lg shadow-md object-contain" /> </div> <div className="flex items-center space-x-2"> <button type="button" onClick={() => previewFileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600"> Change preview </button> <button type="button" onClick={removeCustomPreview} className="px-4 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 dark:hover:bg-slate-700 rounded-md"> Remove preview </button> </div> </div> )} <input type="file" className="hidden" ref={previewFileInputRef} onChange={handleCustomPreviewSelect} accept="image/*" /> </FormSection> </div> ); };


const CreateReel = () => {
  // --- Стан для UI та даних форми (без змін) ---
  ...
  // --- Функції для керування UI (без змін) ---
  ...
  const handleSubmit = async (event) => {
    ...
    if (!commonFormData.craft) {
        alert('Please select a Craft from the common meta data.');
        return;
    }
  ...
  return (
    // Додаємо тег form та обробник onSubmit
    <form onSubmit={handleSubmit} className="max-w-7xl mx-auto space-y-8 pb-36">
      ...
      {/* Оновлена кнопка відправки та індикатор завантаження */}
