...
// --- Компонент для одиночного вибору (ТІЛЬКИ зі списку) ---
...
// --- Компонент для множинного вибору (ТІЛЬКИ зі списку) ---
...
// ✨ --- НОВИЙ КОМПОНЕНТ: Множинний вибір + Створення нових значень --- ✨
const MultiCreatableSelect = ({ label, options, selectedOptions, onChange, placeholder, limit = 10, required = false }) => {
  ...
  const handleAdd = (itemName) => {
    ...
    if (trimmedItem && selectedOptions.length < limit && !selectedOptions.find(opt => opt.toLowerCase() === trimmedItem.toLowerCase())) {
  ...
  useEffect(() => {
    function handleClickOutside(event) { if (wrapperRef.current && !wrapperRef.current.contains(event.target)) { setIsOpen(false); setInputValue(''); } }
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [wrapperRef]);
  ...
  return (
    <FormField label={`${label} (${selectedOptions.length}/${limit})`} required={required}>
      {/* Цей прихований інпут потрібен для нативної валідації HTML5 'required' */}
      <input type="text" value={selectedOptions.join(',')} required={required} className="hidden" onChange={() => {}}/>
      <div className="relative" ref={wrapperRef}>
        <div className="flex flex-wrap items-center gap-2 p-2 min-h-[40px] border border-slate-300 dark:border-slate-700 rounded-md" onClick={() => { setIsOpen(true); wrapperRef.current.querySelector('input[type=text]:not(.hidden)').focus(); }}>
          {selectedOptions.map(option => (
            <span key={option} className="flex items-center bg-teal-100 dark:bg-teal-900/50 text-teal-800 dark:text-teal-200 text-sm font-medium px-2.5 py-1 rounded-full">
              {option}
              <button type="button" onClick={(e) => { e.stopPropagation(); handleRemove(option); }} className="ml-2 -mr-1 p-0.5 text-teal-600 dark:text-teal-300 hover:bg-teal-200 dark:hover:bg-teal-700 rounded-full"> <X size={14} /> </button>
            </span>
          ))}
          {selectedOptions.length < limit && ( <input type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} onFocus={() => setIsOpen(true)} placeholder={placeholder} className="flex-grow bg-transparent focus:outline-none p-1 text-sm" /> )}
        </div>
        {isOpen && (
          <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-md shadow-lg max-h-60 overflow-y-auto">
            <ul onMouseDown={(e) => e.preventDefault()}>
              { canAddCustom && ( <li onClick={() => handleAdd(inputValue)} className="px-3 py-2 text-sm text-teal-600 dark:text-teal-400 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer italic"> Додати "{inputValue.trim()}" </li> )}
              {availableOptions.map((option) => ( <li key={option.id} onClick={() => handleAdd(option.name)} className="px-3 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"> {option.name} </li> ))}
              {!canAddCustom && availableOptions.length === 0 && ( <div className="px-3 py-2 text-sm text-slate-500">No available options</div> )}
            </ul>
          </div>
        )}
      </div>
...
const CreateReel = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    client: [], // Тепер масив
    description: '',
    featuredCelebrity: [], // Тепер масив
    contentType: '',
    craft: '',
    categories: [],
  });

  // --- Решта станів та логіки завантаження без змін ---
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-24">
      <FormSection> <FormField label="Title" required> <input type="text" value={formData.title} onChange={(e) => handleFormChange('title', e.target.value)} placeholder="Enter title..." className={inputClasses} required /> </FormField> </FormSection>
      <FormSection title="Upload Content"> <div onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} className={`relative flex flex-col items-center justify-center w-full aspect-[16/9] border-2 border-slate-300 border-dashed rounded-lg cursor-pointer transition-colors dark:border-slate-600 ${isDragging ? 'border-teal-500 bg-teal-50 dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-800/50'}`}> {!selectedFile ? ( <div className="flex flex-col items-center justify-center text-center"> <UploadIcon /> <p className="mb-2 text-sm text-slate-500 dark:text-slate-400"><span className="font-semibold">Drag & Drop to upload</span></p> <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">or</p> <button type="button" onClick={() => fileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-teal-500 text-white rounded-md hover:bg-teal-600">Choose File</button> </div> ) : ( <div className="w-full h-full p-2"> {mainPreviewUrl && selectedFile.type.startsWith('image/') && <img src={mainPreviewUrl} alt="Preview" className="w-full h-full object-contain rounded-md" />} {mainPreviewUrl && selectedFile.type.startsWith('video/') && <video src={mainPreviewUrl} controls className="w-full h-full object-contain rounded-md" />} <button type="button" onClick={handleRemoveFile} className="absolute top-2 right-2 p-1.5 bg-black/50 text-white rounded-full hover:bg-black/70 transition-colors" aria-label="Remove file"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg> </button> </div> )} </div> <input id="dropzone-file" type="file" className="hidden" ref={fileInputRef} onChange={handleFileInputChange} accept="image/*,video/*" /> </FormSection>
      <FormSection title="Preview"> {!customPreviewUrl ? ( <div className="text-center text-slate-400 py-8"> <p className="mb-4">Here will be shown your preview</p> <button type="button" onClick={() => previewFileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600"> Choose preview </button> </div> ) : ( <div className="flex flex-col items-center"> <div className="w-full flex justify-center mb-4"> <img src={customPreviewUrl} alt="Final Preview" className="w-full h-auto max-w-2xl max-h-[500px] rounded-lg shadow-md object-contain" /> </div> <div className="flex items-center space-x-2"> <button type="button" onClick={() => previewFileInputRef.current.click()} className="px-4 py-1.5 text-xs font-semibold bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-md hover:bg-slate-300 dark:hover:bg-slate-600"> Change preview </button> <button type="button" onClick={removeCustomPreview} className="px-4 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-100 dark:hover:bg-slate-700 rounded-md"> Remove preview </button> </div> </div> )} <input type="file" className="hidden" ref={previewFileInputRef} onChange={handleCustomPreviewSelect} accept="image/*" /> </FormSection>
      <FormSection> <div className="flex items-center"> <input id="allow-download" type="checkbox" checked={formData.allowDownload} onChange={(e) => handleFormChange('allowDownload', e.target.checked)} className="h-4 w-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500" /> <label htmlFor="allow-download" className="ml-3 text-sm font-medium text-slate-700 dark:text-slate-300">Allow download</label> </div> </FormSection>
      ...
      <FormSection title="Content Data">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {/* ✨ ВИКОРИСТАНО НОВИЙ КОМПОНЕНТ ✨ */}
          <MultiCreatableSelect
            label="Artist"
            options={artists}
            selectedOptions={formData.artist}
          ...
          <MultiCreatableSelect
            label="Client"
            options={clients}
            selectedOptions={formData.client}
          ...
          <div className="md:col-span-2">
            <MultiCreatableSelect
                ...
                options={celebrities}
                selectedOptions={formData.featuredCelebrity}
                onChange={(newValue) => handleFormChange('featuredCelebrity', newValue)}
                placeholder={isLoadingOptions ? 'Loading...' : 'Choose celebrity...'}
                limit={10}
            />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
          <div className="md:col-span-2">
            <SingleSearchableSelect
              ...
              onChange={(newValue) => handleFormChange('contentType', newValue)}
              placeholder={isLoadingOptions ? 'Loading...' : 'Choose content type...'}
            />
          </div>
          <div className="md:col-span-2">
            <SingleSearchableSelect
              ...
              onChange={(newValue) => handleFormChange('craft', newValue)}
          ...
          <div className="md:col-span-2">
            <MultiSelectCategories
              ...
              onChange={(newSelection) => handleFormChange('categories', newSelection)}
      ...
      <div className="fixed bottom-6 right-6 z-50"> <button type="submit" className="px-3 py-2 bg-teal-600 text-white text-sm font-semibold rounded-lg shadow-lg hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-opacity-75 transition-transform transform hover:scale-105"> Deploy & Upload </button> </div>
    </div>
  );
};
