...
const ReelPartialForm = ({ reel, onUpdate, onFilesSelected }) => {
  ...
  const isVideo = selectedFile
    ...
    : mainPreviewUrl &&
      !mainPreviewUrl.endsWith('.jpg') &&
      !mainPreviewUrl.endsWith('.png') &&
      !mainPreviewUrl.endsWith('.jpeg') &&
      !mainPreviewUrl.endsWith('.gif');
    ...
    <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-4 space-y-8 relative mb-8">
      ...
      <FormSection title="Preview" hasSeparator={false}>
        {isEditingPreview && isVideo ? (
          ...
          (mainPreviewUrl && !customPreviewUrl) ? (
      ...
      const fetchItemData = async () => {
        ...
        try {
          const response = await fetch(
            `http://localhost:3001/media-items/${itemId}`
          );
  ...
  const handleSubmit = async (event) => {
    ...
    if (isEditMode) {
      ...
      if (reel.selectedFile)
        filesToUpload.push({ file: reel.selectedFile, role: 'main' });
      if (reel.customPreviewFile)
        filesToUpload.push({ file: reel.customPreviewFile, role: 'preview' });
    } else {
      reels.forEach((reel) => {
        if (reel.selectedFile)
          filesToUpload.push({ file: reel.selectedFile, role: 'main' });
      ...
      if (isEditMode) {
        ...
        if (reelToUpdate.selectedFile instanceof File) {
          videoPath = await uploadFileToGCS(
            ...
            'main',
            onProgress(reelToUpdate.selectedFile.name)
          );
        }
        if (reelToUpdate.customPreviewFile instanceof File) {
          ...
          previewPath = await uploadFileToGCS(
            ...
            'preview',
            onProgress(reelToUpdate.customPreviewFile.name)
          );
        }
        ...
        const uploadPromises = reels.map(async (reel) => {
          setUploadStatus((prev) => ({
            ...prev,
            message: `Uploading ${reel.title}...`,
          }));
          const content_gcs_path = await uploadFileToGCS(
            reel.selectedFile,
            'main',
            onProgress(reel.selectedFile.name)
          );
          const preview_gcs_path = reel.customPreviewFile
            ? await uploadFileToGCS(
                reel.customPreviewFile,
                'preview',
                onProgress(reel.customPreviewFile.name)
              )
            : null;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-7xl mx-auto space-y-8 pb-36">
      ...
      <div className="fixed bottom-6 right-6 z-50">
        ...
        <div
          className={`mt-4 flex justify-end ${uploadStatus.isActive ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
        >
          <button
            type="submit"
