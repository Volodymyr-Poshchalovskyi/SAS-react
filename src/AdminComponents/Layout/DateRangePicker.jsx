// src/components/DateRangePicker.jsx (приклад шляху)
...
const DateRangePicker = ({ initialRange, onRangeChange }) => {
  ...
  const [currentMonth, setCurrentMonth] = useState(initialRange.to || new Date());
  ...
  const handleDayClick = (day) => {
    ...
    if (!newRange.from || newRange.to) {
      ...
      newRange.to = day;
      onRangeChange(newRange); // Передаємо фінальний діапазон наверх
      setIsOpen(false); // Закриваємо календар після вибору діапазону
    }
    setRange(newRange);
  };
  ...
  const renderDays = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const isSelectedStart = range.from && date.toDateString() === range.from.toDateString();
      const isSelectedEnd = range.to && date.toDateString() === range.to.toDateString();
      ...
      {isOpen && (
        <div className="absolute top-12 right-0 z-10 w-80 bg-slate-800 border border-slate-700 rounded-xl shadow-lg p-4 text-white">
          <div className="flex items-center justify-between mb-4">
            <button onClick={handlePrevMonth} className="p-2 rounded-full hover:bg-slate-700"><ChevronLeft className="h-4 w-4" /></button>
            <span className="font-semibold text-sm">
              {currentMonth.toLocaleString('en-US', { month: 'long', year: 'numeric' })}
            </span>
            <button onClick={handleNextMonth} className="p-2 rounded-full hover:bg-slate-700"><ChevronRight className="h-4 w-4" /></button>
          </div>
