...
const SortableHeader = ({ children, sortKey, sortConfig, onSort, className = '' }) => {
  ...
  const renderSortIcon = () => {
    ...
    return sortConfig.direction === 'ascending' ? <ArrowUp className="h-4 w-4 text-slate-800" /> : <ArrowDown className="h-4 w-4 text-slate-800" />;
  };
  return (
    <th className={`p-4 font-medium text-left ${className}`}>
      <button className="flex items-center gap-1 group" onClick={() => onSort(sortKey)}>{children} {renderSortIcon()}</button>
    </th>
  );
};
...
const inputClasses = 'flex h-10 w-full rounded-md border border-slate-300 bg-white dark:bg-slate-800 px-3 py-2 text-sm placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 dark:border-slate-700 dark:text-slate-50';
  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={status === 'idle' ? onClose : undefined}>
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        {status === 'success' ? (
          <div className="text-center py-4"><CheckCircle className="h-16 w-16 text-green-500 mx-auto mb-4" /><h3 className="text-lg font-medium text-slate-900 dark:text-slate-50">Deleted Successfully</h3><p className="text-sm text-slate-500 dark:text-slate-400 mt-2">The showreel "<strong className="text-slate-700 dark:text-slate-200">{itemTitle}</strong>" was deleted.</p></div>
        ) : (
          <>
            <div className="flex items-start"><div className="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-red-100 dark:bg-red-900/50 sm:mx-0 sm:h-10 sm:w-10"><AlertTriangle className="h-6 w-6 text-red-600 dark:text-red-400" /></div><div className="ml-4 text-left"><h3 className="text-lg leading-6 font-medium text-slate-900 dark:text-slate-50">Delete Showreel</h3><div className="mt-2"><p className="text-sm text-slate-500 dark:text-slate-400">{status === 'error' ? <span className="text-red-500">Failed to delete. Please try again.</span> : <>Are you sure you want to delete <strong className="text-slate-700 dark:text-slate-200">{itemTitle}</strong>? This action cannot be undone.</>}</p></div></div></div>
            <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse gap-3">
              <button type="button" onClick={handleConfirmClick} disabled={status === 'deleting'} className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 sm:w-auto sm:text-sm disabled:bg-red-400"><Loader2 className={`animate-spin h-5 w-5 ${status !== 'deleting' && 'hidden'}`} /> {status !== 'deleting' && 'Delete'}</button>
              <button type="button" onClick={onClose} disabled={status === 'deleting'} className="mt-3 w-full inline-flex justify-center rounded-md border border-slate-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-700 text-base font-medium text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 sm:mt-0 sm:w-auto sm:text-sm disabled:opacity-50">Cancel</button>
            </div>
          </>
        )}
      </div>
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50" onClick={status === 'idle' ? onClose : undefined}>
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        <form onSubmit={handleSave}>
          <h3 className="text-lg leading-6 font-medium text-slate-900 dark:text-slate-50 mb-4">Edit Showreel</h3>
          <div className="space-y-4">
            ...
            <FormField label="Short Link">
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={handleCopyLink} title="Copy Link" className="p-2 rounded-md border border-slate-300 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700">
  ...
  return (
    <div ref={dropdownRef} className={`absolute right-4 ${positionClass} z-20 w-48 bg-white dark:bg-slate-800 rounded-md shadow-lg border border-slate-200 dark:border-slate-700`}>
      <ul className="py-1 text-sm text-slate-700 dark:text-slate-200">
        <li><button onClick={() => { onEdit(reel); onClose(); }} className="w-full flex items-center px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-700"><Edit className="mr-3 h-4 w-4" /><span>Edit</span></button></li>
...
const MyAnalytics = () => {
  ...
  const handleSaveSuccess = (updatedReel) => {
    setReelsData(prevData => prevData.map(reel => reel.id === updatedReel.id ? updatedReel : reel));
  };
                      ...
                      <td className="p-4 text-center">
                         <div className="relative flex items-center justify-center">
                            ...
                            {activeDropdown === reel.id && (
                                <ReelActionsDropdown reel={reel} onEdit={handleEdit} onCopy={handleCopy} onDelete={setReelToDelete} onToggleStatus={handleToggleStatus} onClose={() => setActiveDropdown(null)} isLastItem={isLastItem} />
                            )}
                         </div>
                      </td>
                    </tr>
