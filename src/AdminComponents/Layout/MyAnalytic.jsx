...
const MyAnalytics = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
        ...
        console.error(error);
        // Відкат змін в UI у разі помилки на сервері
          ...
          <table className="w-full text-sm table-fixed">
            <thead className="text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900">
              <tr className="border-b border-slate-200 dark:border-slate-800">
                <SortableHeader sortKey="title" sortConfig={sortConfig} onSort={handleSort} className="w-[40%]">Reel Title</SortableHeader>
                <SortableHeader sortKey="created_at" sortConfig={sortConfig} onSort={handleSort} className="w-[20%]">Created At</SortableHeader>
                <SortableHeader sortKey="status" sortConfig={sortConfig} onSort={handleSort} className="w-[15%] text-center">Status</SortableHeader>
                <th className="p-4 font-medium text-center w-[15%]">Short Link</th>
            ...
            <tbody className="text-slate-800 dark:text-slate-200 divide-y divide-slate-100 dark:divide-slate-800">
              {currentData.map((reel) => (
                <tr key={reel.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                  <td className="p-4 text-left font-medium text-slate-900 dark:text-slate-50 truncate">
                    <Highlight text={reel.title} highlight={searchTerm} />
                  </td>

                  <td className="p-4 text-left text-slate-600 dark:text-slate-400 truncate">{formatDateTime(reel.created_at)}</td>
